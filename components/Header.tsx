    ...
    >
      <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center gap-4">
            ...
            <a href="#booking" className="bg-transparent border border-aviation-light text-aviation-light hover:bg-aviation-light hover:text-aviation-blue font-bold py-2 px-6 rounded-sm uppercase tracking-wider transition-all duration-300 shadow-[0_0_10px_rgba(102,252,241,0.1)] hover:shadow-[0_0_20px_rgba(102,252,241,0.4)]">
                Book Now
            </a>
        </div>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-aviation-gray border-t border-white/10 shadow-xl py-6 px-4 flex flex-col gap-4">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.label}
              href={item.href}
          ...
          <a href="#booking" onClick={() => setIsMobileMenuOpen(false)} className="w-full bg-aviation-light text-center block text-aviation-blue font-bold py-3 mt-4 rounded-sm uppercase">
            Book Now
          </a>
        </div>
      )}
